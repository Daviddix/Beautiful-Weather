...
import SearchResultError from "../ErrorComponents/SearchResultError/SearchResultError"
  ...
  async function innerFetch(searchParam: string) {
    try {
      const rawFetch = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?q=${searchParam}&units=metric&limit=3&appid=${import.meta.env.VITE_API_KEY}`
      );
